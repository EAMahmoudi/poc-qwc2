...
updateTsConfig(process.cwd() + '/src/qwc2-demo-app/js', '/src/qwc2-demo-app/static/translations/tsconfig.json', collectedMsgIds);
...
for (const lang of config.languages || []) {
    ...
    const origData = readJSON('/src/qwc2-demo-app/static/translations/' + lang + '.json');
    let data = merge(langskel, cleanMessages(readJSON('/src/qwc2-demo-app/static/translations/' + lang + '.json'), langskel));
    ...
    // Write output
    try {
        fs.writeFileSync(process.cwd() + '/src/qwc2-demo-app/static/translations/' + lang + '.json', JSON.stringify(data, null, 2) + "\n");
