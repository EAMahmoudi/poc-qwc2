...
function getThumbnail(configItem, resultItem, layers, crs, extent, resolve, proxy) {
    if (configItem.thumbnail !== undefined) {
        ...
        if (fs.existsSync("./src/qwc2-demo-app/static/assets/img/mapthumbs/" + configItem.thumbnail)) {
    ...
    axios.get(getMapUrl, {
        ...
        try {
            fs.mkdirSync("./src/qwc2-demo-app/static/assets/img/genmapthumbs/");
        } catch (err) {
            if (err.code !== 'EEXIST') throw err;
        }
        fs.writeFileSync("./src/qwc2-demo-app/static/assets/img/genmapthumbs/" + basename, response.data);
...
function genThemes(themesConfig) {
    ...
    Promise.all(tasks).then(() => {
        ...
        // write config file
        fs.writeFile(process.cwd() + '/src/qwc2-demo-app/static/themes.json', JSON.stringify(result, null, 2), (error) => {
            if (error) {
